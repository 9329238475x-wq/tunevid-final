...
const DeadlineProgress: React.FC<DeadlineProgressProps> = ({ progress, statusMsg }) => {
    // Map 0-100 progress to the values used in the original animation
    // Original fill x goes from -100 to -3
    const fillX = -100 + (progress * 0.97);
    // Original death-group translateX goes from 0 to 520
    const reaperX = (progress / 100) * 520;

    return (
        <div className="deadline-container w-full max-w-[581px] mx-auto overflow-hidden">
            <div id="deadline" className="relative w-full h-[158px]">
                <svg preserveAspectRatio="xMinYMin meet" id="line" viewBox="0 0 581 158" className="w-full h-full">
                    {/* Fire Group */}
                    <g id="fire">
                        <rect id="mask-fire-black" x="511" y="41" width="38" height="34" fill="#000" />
                        <g>
                            <defs>
                                <rect id="mask_fire" x="511" y="41" width="38" height="34" />
                            </defs>
                            <clipPath id="mask-fire_react">
                                <use href="#mask_fire" overflow="visible" />
                            </clipPath>
                            <g id="group-fire" clipPath="url(#mask-fire_react)">
                    ...
                    {/* Grim Reaper (Death) */}
                    <g id="death-group-dyn" style={{ transform: `translateX(${reaperX}px)`, transition: 'transform 0.5s ease' }}>
                        ...
                        <path id="death-arm" className="animate-death-arm" fill="#BE002A" d="M-53.375,75.25c0,0,9.375,2.25,11.25,0.25s2.313-2.342,3.375-2.791 c1.083-0.459,4.375-1.75,4.292-4.75c-0.101-3.627,0.271-4.594,1.333-5.043c1.083-0.457,2.75-1.666,2.75-1.666 s0.708-0.291,0.5-0.875s-0.791-2.125-1.583-2.959c-0.792-0.832-2.375-1.874-2.917-1.332c-0.542,0.541-7.875,7.166-7.875,7.166 s-2.667,2.791-3.417,0.125S-49.833,61-49.833,61s-3.417,1.416-3.417,1.541s-1.25,5.834-1.25,5.834l-0.583,5.833L-53.375,75.25z" style={{ transformOrigin: '-60px 74.5px' }} />
                        <path id="death-tool" className="animate-death-tool" fill="#BE002A" d="M-20.996,26.839l-42.819,91.475l1.812,0.848l38.342-81.909c0,0,8.833,2.643,12.412,7.414 c5,6.668,4.75,14.084,4.75,14.084s4.354-7.732,0.083-17.666C-10,32.75-19.647,28.676-19.647,28.676l0.463-0.988L-20.996,26.839z" style={{ transformOrigin: '-48px 74.5px' }} />
                    </g>
                    ...
                    <g id="designer-arm-grop" className="animate-designer-arm" style={{ transformOrigin: '90% top' }}>
                        ...
                        <path id="designer-pen" fill="#FEFFFE" d="M525.75,59.084c0,0-0.423-0.262-0.969,0.088c-0.586,0.375-0.547,0.891-0.547,0.891l7.172,8.984l1.261,0.453 l-0.104-1.328L525.75,59.084z" />
                    </g>
                </svg>
            </div>

            <div className="text-center mt-4">
                <p className="text-rose-500 font-black text-xl italic uppercase tracking-tighter" style={{ fontFamily: "'Oswald', sans-serif" }}>
            ...
            <style jsx>{`
        .animate-death-arm {
          animation: move-arm 2s ease infinite;
        }
        .animate-death-tool {
          animation: move-tool 2s ease infinite;
        }
        .animate-designer-arm {
          animation: write 1s ease infinite;
        }
        #red-flame, #yellow-flame, #white-flame {
          opacity: ${progress > 90 ? 1 : 0};
          transform-origin: center bottom;
        }
        #red-flame { animation: red-flame 120ms ease infinite; }
        #yellow-flame { animation: yellow-flame 120ms ease infinite; }
        #white-flame { animation: red-flame 100ms ease infinite; }
        ...
        @keyframes write {
          0%, 32%, 65%, 100% { transform: rotate(0deg) scale(1, 1); }
          16% { transform: rotate(5deg) scale(0.8, 1); }
          48% { transform: rotate(6deg) scale(0.8, 1); }
          83% { transform: rotate(4deg) scale(0.8, 1); }
        }
        @keyframes red-flame {
          0%, 100% { transform: translateY(-30px) scale(1, 1); }
          25% { transform: translateY(-30px) scale(1.1, 1.1); }
          75% { transform: translateY(-30px) scale(0.8, 0.7); }
        }
        @keyframes yellow-flame {
          0%, 100% { transform: translateY(-30px) scale(1, 1); }
          50% { transform: translateY(-30px) scale(1.1, 1.2); }
          75% { transform: translateY(-30px) scale(0.8, 0.7); }
        }
      `}</style>
        </div>
